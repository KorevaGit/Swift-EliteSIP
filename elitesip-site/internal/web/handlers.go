...
func (s *Server) saveEmployee(w http.ResponseWriter, r *http.Request, admin model.Admin) {
	...
	// Что было до правки — нужно, чтобы отличить «сменили номер» от
	// «поправили опечатку в имени». Первое обесценивает невыданные ключи,
	// второе не значит для них ничего.
	before, beforeErr := s.DB.EmployeeByID(r.Context(), id)
	...
	// Номер и SIP-пароль застывают в пакете активации в момент выпуска, и
	// правка карточки их не догоняет: ключ, выданный вчера и введённый
	// завтра, поднял бы машину со старым добавочным — молча и с виду успешно.
	// Раньше об этом говорила строка во всплывающем сообщении; строку легко не
	// увидеть, а цена промаха — рабочее место под чужим номером. Теперь
	// невостребованные ключи гасятся сами. Аудит 27 августа 2026.
	//
	// Предустановка сюда не входит намеренно: её новую ревизию настроенная
	// машина забирает сама следующим заходом на канал, и гасить ради неё
	// ключи значило бы наказывать за обычную правку раскладки.
	stale := 0
	if beforeErr == nil && (before.Number != e.Number || before.SIPPassword != e.SIPPassword) {
		var err error
		stale, err = s.Issuer.StalePending(r.Context(), actorOf(admin), id, "данные карточки изменились")
		if err != nil {
			s.flash(r, "bad", "Карточка сохранена, но старые ключи ещё работают",
				friendly(err)+" Выданные и не использованные ключи поднимут машину с прежним номером — отзовите их вручную.")
			s.back(w, r, back)
			return
		}
	}
	...
	case stale == 1:
		s.flash(r, "ok", "Карточка сохранена",
			"Невостребованный ключ погашен: он нёс прежний номер. Выпустите новый, если он был нужен. "+
				"Предустановка на настроенных машинах поменяется сама из файла на R2.")
	case stale > 1:
		s.flash(r, "ok", "Карточка сохранена",
			fmt.Sprintf("Невостребованных ключей погашено: %d — они несли прежний номер. "+
				"Выпустите новый, если он был нужен. "+
				"Предустановка на настроенных машинах поменяется сама из файла на R2.", stale))
	default:
		s.flash(r, "ok", "Карточка сохранена",
			"Предустановка на настроенных машинах поменяется сама из файла на R2.")
	}

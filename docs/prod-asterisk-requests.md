...
## 5. ~~Не Asterisk, а инфраструктура~~ — отвечено по SIP, открыто по панели

Пришло 3 августа 2026: удалённый профиль не проходит ни через какой туннель.
Скрипт подключения делает стук по портам ICMP-пакетами заданной длины, шлюз —
MikroTik. L2TP/IPsec на шлюзе есть, но только для айфонов с PortSIP. Разбор и
принятые решения — [remote-access.md](remote-access.md).

Открытым остаётся одно: доступна ли панель EliteSIP (политика, макросы, телеметрия)
удалённым сотрудникам и каким именно путём. Скрипт стучит по трём адресам, и
какой из них чей — неизвестно.

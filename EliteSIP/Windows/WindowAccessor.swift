...
final class PanelHeightView: NSView {
    ...
    func apply(height: CGFloat) {
        ...
        // восстанавливается по имени (`setFrameUsingName`) **после** первого
        // применения, вместе с высотой, сохранённой прошлой версией
        // приложения. Расчёт при этом не менялся, выход срабатывал, и панель
        // навсегда оставалась той высоты, какой её сохранили когда-то, — с
        // пустой полосой под кнопкой звонка. Нашёл живой прогон 19 августа 2026.
        //
        ...
        // Панель меняет высоту чаще, чем обычное окно, — при каждом ряде
        // макросов, при удержании, при появлении поля перевода. На Big Sur
        // WindowServer изредка не пересчитывает рамку и тень окна вслед за
        // частым программным `setFrame`: полоса заголовка со светофором
        // перерисовывается (она внутри содержимого), а системная обводка
        // вокруг всего окна — нет, и остаётся от кадра до следующей смены
        // экрана или полного перерисовывания. Явный запрос на пересчёт тени
        // после каждого `setFrame` не даёт этому кадру застрять.

...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    /// Приложение **всегда обычное**: иконка в Dock и настоящая строка меню.
    ///
    /// До 17 августа 2026 форма зависела от окон: панель на экране — `.regular`,
    /// закрыли всё — `.accessory`, то есть приложение сворачивалось в значок в
    /// строке меню. Это решение этапа 6, и отменил его заказчик по причине,
    /// которую с одного монитора не увидеть: **значков в строке меню у него
    /// столько, что нашего там не найти.** Значок был единственной дорогой к
    /// закрытой панели — то есть дорога была невидимой, а телефон при этом
    /// оставался на линии и принимал звонки.
    ///
    /// Иконка в Dock решает это без выбора и без настройки: она видна всегда, и
    /// щелчок по ней возвращает панель (`applicationShouldHandleReopen`).
    ///
    /// Значок в строке меню при этом остаётся — он показывает состояние
    /// регистрации и разговора, и у кого строка меню посвободнее, тот им и
    /// пользуется. `LSUIElement` в `Info.plist` по-прежнему не ставится: он задал
    /// бы стартовую политику, а стартуем мы обычным приложением.
    ///
    /// Метод оставлен вызываемым из мест показа окон, хотя теперь только
    /// подтверждает форму: сокращать его до «сделать один раз при запуске»
    /// значило бы разложить одно решение по десятку мест.
    private func updateActivationPolicy() {
        guard NSApp.activationPolicy() != .regular else { return }
        NSApp.setActivationPolicy(.regular)
        // Переход в обычное приложение не выводит его вперёд сам: без этого окно
        // открывается за чужим, и человек, нажавший «Настройки», видит прежнюю
        // программу.
        NSApp.activate(ignoringOtherApps: true)
    }
    ...
    private func showFirstRunWindow(
        ...
        // Размер — **до** восстановления кадра, а не после.
        //
        // Тот же урок, что записан у панели, и повторённый здесь в обратном
        // порядке. `NSHostingController` подгоняет окно под содержимое не сразу, а
        // следующим проходом раскладки: в момент `setFrameUsingName` окно ещё
        // нулевого размера, и восстановление кадра ложится в никуда — замер
        // 17 августа 2026 дал `restored=true frame=(684, 5, 0, 0)`, а настоящий
        // размер приезжал асинхронно, уже на главном мониторе. Снаружи это
        // выглядело как «окно прыгает после переключения языка», и двух попыток
        // починить это сохранением кадра не хватило: сохранялся-то он верно.
        window.setContentSize(
            CGSize(width: Theme.Metrics.firstRunWidth, height: Theme.Metrics.firstRunHeight)
        )

        // Кадр запоминается, хотя окно одноразовое: мастер перезапускает
        // приложение сам — после выбора языка, — и без этого окно возвращалось не
        // туда, где стояло. На одном экране незаметно, на двух перепрыгивает на
        // другой монитор посреди настройки.
        restoreFrame(
            of: window,
    ...
    private func makeSidebarWindow<Sidebar: View, Content: View>(
        ...
        if isGlass, #available(macOS 11.0, *) {
            ...
            //
            // Только под стекло, и это выяснилось попыткой сделать иначе.
            // 19 августа 2026 полная высота была включена и без стекла — чтобы
            // убрать пустую полосу над сайдбаром на Big Sur. Одной этой
            // настройки мало: полную высоту система даёт только вместе с
            // `.fullSizeContentView` и непустой панелью инструментов, а панель
            // сама по себе выше той полосы, от которой уходили. Полоса выросла
            // вдвое. Без стекла окно осталось обычным: полоса заголовка своя,
            // содержимое под ней.
        ...
        // содержимое заводится под полосу заголовка. Без стекла окно обычное —
        // полоса заголовка своя, содержимое под ней; полная высота сайдбара там
        // требует ещё и панели инструментов, которая выше самой полосы.
        ...
        // окна. Проверено 19 августа 2026: с панелью полоса над сайдбаром стала
        // вдвое выше той, из-за которой всё и затевалось.
        if isGlass, #available(macOS 11.0, *) {

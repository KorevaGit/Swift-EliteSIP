...
enum Theme {
    ...
    enum Metrics {
        ...
        ///
        /// 520 — по самому длинному, что в ней стоит: имя собеседника, под ним
        /// номер с меткой профиля и пометками перевода, справа время с исходом
        /// и кнопка повторного набора. На меньшей ширине первым ужимается
        /// подзаголовок, то есть ровно то, чем номер отличается от соседнего.
        static let historyMinWidth: CGFloat = 520

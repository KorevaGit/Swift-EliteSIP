...
struct PhonePanelView: View {
    ...
    private func reportMiddleHeight(_ height: CGFloat) {
        ...
        // Промежуточные проходы раскладки в расчёт не идут.
        //
        // Пока окно не получило свою высоту, середина меряется в считаные
        // точки, и нехватка выходит огромной. Добавка от такого замера
        // задирала окно до потолка за один проход — это и увидела живая
        // проверка 19 августа 2026. Настоящая нехватка исчисляется десятками
        // точек, а не половиной панели.
        guard height > expected / 2 else { return }
    ...
    private var middle: some View {
        ...
        VStack(spacing: 0) {
            ...
            // Куда девается лишняя вертикаль, если окно оказалось выше расчёта.
            // Вниз, а не в клавиши: воздух под сеткой глаз не замечает вовсе, а
            // растянутая клавиша читается как поломка.
            Spacer(minLength: 0)
        }

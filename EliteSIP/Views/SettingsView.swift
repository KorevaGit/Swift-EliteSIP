    ...
    var body: some View {
        Form {
            ...
            Section(header: Text("Полоса")) {
                ...
                MilestoneNote("""
                    50–7000 Гц вместо 300–3400 при том же битрейте. На нашей АТС \
                    выключатель сейчас ничего не меняет: сервер выбирает кодек по \
                    своему порядку и всегда берёт G.711. G.722 остаётся в предложении \
                    последним — на случай, если настройку АТС когда-нибудь поменяют.
                    """)
            }

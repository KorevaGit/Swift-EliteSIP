...
final class HistorySnapshotAnchor {
    ...
    func image(profile: String) throws -> NSImage {
        ...
        //
        // Здесь он включался ровно один день. На macOS 26 всё окно слоёное само,
        // на Big Sur — нет, и `wantsLayer` ставился по требованию, перед
        // съёмкой. Живой прогон 20 августа 2026 показал цену: включение слоя
...
final class HistorySnapshotAreaView: NSView {
    ...
    /// Вьюха невидима для мыши целиком.
    ///
    /// Она существует ради одного — сообщить якорю свои границы, — и рисует
    /// ровно ничего. А вот в разборе нажатий она участвовала, и на Big Sur это
    /// стоило кнопок: `NSViewRepresentable`, поставленный фоном, попадает в
    /// обычный AppKit hit-testing и имеет там преимущество перед тем, что
    /// SwiftUI рисует поверх. Кадр у фона разрешается не всегда там, где
    /// ожидаешь — плашка стоит с `maxHeight: .infinity`, — и накрытый ряд
    /// кнопок переставал нажиматься весь разом. На macOS 26 путь другой, и
    /// потому воспроизводилось это только на Big Sur и только иногда.
    ///
    /// `nil` снимает вопрос целиком, не отнимая ничего: границы читаются из
    /// `bounds`, а не из нажатий. Найдено аудитом 27 августа 2026.
...
private struct HistorySnapshotButton: View {
    ...
    private func copy() {
        ...
        // Одна запись с двумя представлениями, а не несколько записей подряд:
        // `setData` и `writeObjects` кладут в буфер разные элементы, и часть
        // приложений берёт из него первый попавшийся.
        //
        // **Ссылки на файл здесь больше нет, и её убрали намеренно.** Она
        // появилась 24 августа 2026 по догадке «мессенджеры разбирают вставку
        // как вложение и ищут в буфере файл». Догадка была неверна дважды.
        // Во-первых, ссылка на файл, положенная в буфер обмена, **не несёт
        // sandbox-расширения** — в отличие от перетаскивания, — и получатель
        // из песочницы (Telegram из App Store) до файла во временном каталоге
        // не дотягивается вовсе. Во-вторых, увидев ссылку, он перестаёт
        // смотреть на картинку: вставка разбирается как вложение, вложение не
        // читается, и не вставляется ничего. То есть правка не лечила отказ, а
        // создавала его. Разобрано аудитом 27 августа 2026.
        //
        // Порядок представлений — тот же, что кладёт системный снимок экрана:
        // TIFF первым, PNG вторым. Не из почтения к системе, а потому что этот
        // порядок на этой же машине заведомо вставляется в переписку, а наш —
        // нет.
...
private struct CallHistoryRow: View {
    ...
    ///
    /// Обычно это `record.title` — имя, а нет имени, номер. Два исключения, и
    /// оба берут правило у `IncomingCallSubject`, а не переписывают его здесь:
    /// сравнение по цифрам и сами заголовки обязаны совпадать с окном входящего
    /// слово в слово, иначе один и тот же звонок называется по-разному в двух
    /// местах приложения.
    ///
    /// **Свой добавочный** — звонок по сделке: Битрикс поднимает менеджера с
    /// его номера и только потом набирает клиента. В окне входящего он давно
    /// подписан словами; в истории оставался номер — строка «176» в списке
    /// своих звонков ничего не объясняла. Живая проверка 19 августа 2026.
    ///
    /// **Номер из словаря очередей** — раздача. Правило то же самое,
    /// исключение то же самое: администратор называет очередь словами
    /// («🔥Горячая раздача🔥»), а история показывала цифры, которые он же и
    /// вписывал в словарь, чтобы их не показывать. Живая проверка 20 августа
    /// 2026 — до неё словарь был передан пустым нарочно, «в истории они и
    /// раньше стояли номером»; довод не пережил проверки. Номер при этом не
    /// теряется — он остаётся ниже, в `subtitle`.
    ///
...
private struct HistoryCalendar: View {
    ...
    /// Календарь окна — **один на сетку и на шапку**.
    ///
    /// Разделять их нельзя: неделя начинается там, где решит этот объект, и
    /// второй ответ на тот же вопрос немедленно даёт сдвиг подписей (см.
    /// `weekdayTitles`).
    private var calendar: Calendar {
        var calendar = Calendar(identifier: .gregorian)
        // Локаль — ради названий: от неё зависят сокращения дней и имя месяца.
        // Первый день недели она тоже приносит, но он тут же переопределяется
        // ниже, и это осознанно.
        calendar.locale = .autoupdatingCurrent
        // Неделя с понедельника — независимо от региона системы. Рабочая
        // неделя колл-центра идёт с понедельника по пятницу, и в сетке,
        // начатой с воскресенья, она разорвана пополам: суббота с
        // воскресеньем оказываются по разным краям строки.
        //
        // Цена названа прямо: у человека с регионом США календарь начнётся не
        // там, где он привык. Принято ради того, ради чего календарь и
        // открывают, — найти рабочий день.
    ...
    /// Сокращения дней в том порядке, в каком стоят колонки сетки.
    ///
    /// **Тем же `calendar`, что и сетка, а не `Calendar.current`.** Разными они
    /// были до 18 августа 2026, и это давало сдвиг на клетку везде, где регион
    /// системы начинает неделю не с понедельника: сетка строится от
    /// `firstWeekday = 2` жёстко, а шапка поворачивалась по системному значению.
    /// При регионе США (`firstWeekday == 1`) подписи уезжали на воскресенье, и
    /// все семь чисел вставали под чужими днями недели — на полностью русском
    /// окне тоже, потому что регион и язык интерфейса это разные настройки.
    ///
    /// Свойство осталось вычисляемым от `calendar`, а не статическим: статику
    /// нельзя связать с экземпляром, а связь здесь и есть весь смысл правки.
    ...
    private var weekdays: some View {
        HStack(spacing: 0) {
            // Сами сокращения берутся у календаря, а не переводом: они зависят
            // от языка, и список из семи строк пришлось бы переводить руками.
            // Порядок при этом наш — см. `weekdayTitles`.

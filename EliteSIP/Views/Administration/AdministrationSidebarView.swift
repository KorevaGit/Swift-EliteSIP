        ...
        List(selection: selection) {
            ForEach(AdministrationSection.groups) { group in
                if let title = group.title {
                    ...
                    // Безымянная группа кладётся в список голым `ForEach`, без
                    // `Section`.
                    //
                    // `Section` без заголовка на macOS 26 не занимает ничего, а
                    // на Big Sur резервирует место под заголовок, которого нет:
                    // живая проверка 19 августа 2026 показала над первой
                    // строкой сайдбара пустую полосу примерно в три десятка
                    // точек. Поля сверху, ради которых обёртка ставилась,
                    // задаёт `compatOwnTopInset` ниже — и задаёт одинаково
                    // обоим окнам, а не по-разному в зависимости от системы.
                    rows(group.items)
                }
            }
        }

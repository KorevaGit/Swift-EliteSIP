...
struct ProfileCard: View {
    ...
    private var form: some View {
        ...
        // Пара адресов — здесь, а не только в разделе «АТС».
        //
        // Профиль заводят одним заходом: номер, пароль, домен. Домен при этом
        // один, а адресов у той же АТС два — изнутри и снаружи, — и узнать об
        // этом можно было, только уйдя в другой раздел окна. На практике туда
        // не уходили: профиль заводился с одним адресом, и переключатель
        // «Работы» у менеджера потом не делал ничего.
        //
        // Правится общая пара машины, а не своя у каждого профиля: у машины
        // одна АТС, а два профиля на ней — это два номера одного человека.
        // Подпись говорит это прямо, чтобы правка «своего» профиля не выглядела
        // местной.
        SettingsRow("Адрес из офиса") {
        ...
        SettingsNote("""
            Пара общая для всей машины — та же, что в разделе «АТС». По ней менеджер \
            переключает себя между офисом и домом разделом «Работа»: домен АТС профиля \
            меняется на парный. Профиль, у которого домен не из этой пары, переключение \
            не трогает.
            """)

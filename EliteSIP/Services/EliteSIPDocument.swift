...
enum EliteSIPDocument {
    ...
    static func encode(config settings: AppSettings) throws -> Data {
        // Чистка делается **при записи**, а не только при чтении.
        //
        // Читающая сторона всё равно выбрасывает то же самое
        // (`machineIndependent`), и одно время этого казалось достаточно. Но
        // тогда в файле лежало больше, чем он обещает: конфигурация — это
        // слепок рабочего места, а не машины, и оформление с путём к своему
        // рингтону в нём не место. Разбирающий такой файл глазами видел поля,
        // которых по описанию там нет, и справедливо переставал ему верить.
        //
        // Что остаётся: профили с номером и паролем SIP, клавиши, очереди,
        // приём вызова, АТС, стук, журнал, история. Что уходит: звуковые
        // устройства и свой рингтон (принадлежат железу), оформление
        // (принадлежит вкусу того, кто за машиной сидит) и блок
        // административного доступа целиком.
        let portable = machineIndependent(settings)
    ...
    /// Всё, что принадлежит рабочему месту, — без того, что принадлежит машине.
    ///
    /// Зовётся с обеих сторон: при записи файла и при чтении. Дважды не из
    /// недоверия к себе, а потому что файл могли записать прежней версией или
    /// собрать снаружи — EliteDash в M9 будет делать ровно это.
    static func machineIndependent(_ settings: AppSettings) -> AppSettings {
        var result = settings

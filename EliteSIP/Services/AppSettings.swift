...
struct AppSettings: Codable, Sendable, Equatable {
    ...
    /// Показывать приложение без стекла — так, как оно выглядит на системах до
    /// macOS 26.
    ///
    /// Оформлений два (`Theme.Chrome`), и обычно вариант выбирает версия
    /// системы. Это — ручной перевод на обычный, и не для одного окна: матовые
    /// поверхности вместо стекла во всех окнах, а в «Управлении» ещё и список
    /// слева на системном фоне с разделителем вместо плавающей вставки.
    ///
    /// Нужен по двум причинам. Первая: обычный вариант надо кому-то смотреть
    /// глазами, а на машине с Tahoe он иначе не показывается вовсе — и до
    /// первой установки на Catalina мы о нём знаем только то, что он собрался.
    /// Вторая: стекло нравится не всем, а окно администратора открывают надолго.
    ///
    /// Обратного перевода нет: включить стекло там, где его нет в системе,
    /// нельзя, и на старых системах этот переключатель ничего не меняет.
    ///
    /// Схема не выросла: старый файл читается терпимым декодером и получает
    /// `false` — ровно прежнее поведение, когда вариант выбирала только версия.
    var plainChrome: Bool = false

    /// Предустановки — сохранённые копии настроек рабочего места.
    ///
    /// Схема не выросла: старый файл читается терпимым декодером и получает
    ...
    /// На каком месте стоит мастер первоначальной настройки (этап 9).
    ///
    /// Умолчание — `.passed`, и это главное в поле. Мастер показывается по
    /// **отсутствию файла настроек**, а не по этому признаку: у машин беты файл
    /// есть и признака в нём нет, и умолчание `.needed` погнало бы их всех в
    /// мастер при первом же обновлении — требовать административный пропуск у
    /// сотрудников, которые его не знают.
    ///
    /// Само поле нужно ровно для двух случаев, когда файл есть, а мастер не
    /// пройден: `.awaitingFinale` — всё применено и приложение перезапустилось
    /// ради языка и стекла, остался последний экран; `.needed` — машину сбросили,
    /// и настройки записались обратно умолчаниями.
    var firstRun: FirstRunStage = .passed

    /// Что машина знает о панели EliteSIP (M9).
    ///
    /// Всё, что здесь лежит, приезжает пакетом активации и живёт до сброса
    /// машины. Пустой блок означает «машину поднимали не ключом» — вручную, из
    /// файла или предустановкой, — и это обычное состояние, а не недонастройка.
    var panel: PanelSettings = PanelSettings()
    ...
    init(from decoder: Decoder) throws {
        ...
        // терпимый декодер молча оставлял его нетронутым: тема записывалась в
        // файл синтезированным `encode(to:)` и терялась при следующем запуске.
        // Снаружи это выглядело как «настройка темы не работает» — и так и было.
        appearance = try container.decodeIfPresent(
            AppearanceSetting.self, forKey: .appearance
        ) ?? .system
        plainChrome = try container.decodeIfPresent(Bool.self, forKey: .plainChrome) ?? false
        // Испорченная предустановка читается как «их нет», а не роняет весь
        ...
        // терпимо, как у `site`: файл правят руками, и опечатка в нём не должна
        // запирать приложение мастером.
        firstRun =
            (try? container.decodeIfPresent(FirstRunStage.self, forKey: .firstRun))
            .flatMap { $0 } ?? .passed
        // Испорченный блок читается как «панели нет», а не роняет файл: по той
        // же причине, что блок доступа и предустановки. Машина, потерявшая
        // связь с панелью, продолжает звонить — это прямо записано в M7c и в
        // границах M9.
        panel = (try? container.decodeIfPresent(PanelSettings.self, forKey: .panel)) ?? PanelSettings()
    ...
    ///
    /// **`*3` — это не догадка, а то, чем контора пользуется сегодня.**
    /// 18 августа 2026 пришли два документа, и они противоречат друг другу.
    /// `core show features` с боевого сервера конференции не содержит вовсе:
    /// там перехват `*8`, слепой перевод `##`, консультационный `*02`, отбой
    /// `**`, запись `*1` и отмена перевода `*77`. А настройки боевого MicroSIP
    /// показывают горячую клавишу «КОНФЕРЕНЦИЯ» с типом DTMF и значением `*3`
    /// — рядом с макросами `*029999#`, которые заведомо работают.
    ///
    /// Объяснить это расхождение мы не можем, и врать в комментарии не будем.
    /// Но ставить своё объяснение выше рабочей настройки заказчика нельзя:
    /// один раз это уже сделали в обратную сторону — сняли `*3` как
    /// «опровергнутый», — и сняли ровно то, что у людей работает.
    ///
    /// Поэтому значение прежнее, а разбор — в docs/conference.md.
    ...
    struct DTMFSettings: Codable, Sendable, Equatable {
        ...
        /// оставаться константой установки (принцип 3). `MacroGrid` кладёт их по
        /// три в ряд, ряд стоит 64 точки вместе с промежутком, и восемнадцать
        /// дают шесть рядов — 586 точек содержимого против 193 у панели без
        /// макросов. Это принято сознательно: на экране 1280×800 панель займёт
        /// около трёх четвертей высоты и станет тесной для замысла «поверх чужой
        /// CRM», но останется предсказуемой.
        ///
        /// Порядок хранится самим массивом: JSON его сохраняет, и отдельного
        /// поля под номер позиции не нужно.
        static let maximumMacros = 18
...
/// На каком месте стоит мастер первоначальной настройки.
///
/// Три состояния, а не флаг: «не пройден» и «остался последний экран» ведут себя
/// по-разному, и различать их приходится ровно потому, что между экраном
/// «Стекло» и финалом приложение перезапускается — язык и корпус берутся при
/// старте процесса и иначе не применяются.
enum FirstRunStage: String, Codable, Sendable, Equatable {

    /// Машину надо настроить с начала. Ставится сбросом машины: настройки после
    /// него записываются умолчаниями, файл появляется снова, и по отсутствию
    /// файла мастер уже не позвать.
    case needed

    /// Язык выбран, приложение перезапустилось, мастер продолжается со второго
    /// экрана.
    ///
    /// Перезапуск стоит сразу после выбора языка — по решению 17 августа 2026.
    /// Иначе выбранный язык не показывался до самого конца: он берётся при
    /// старте процесса (`AppleLanguages`), и мастер шёл на том, что угадала
    /// система. Момент для перезапуска идеальный: на первом экране, кроме языка,
    /// человек ещё ничего не ввёл, и терять при перезапуске нечего.
    case languageChosen

    /// Мастер пройден. Умолчание для файлов, в которых признака нет вовсе.
    case passed
}
...
enum SettingsStore {
    ...
    /// Есть ли файл настроек вообще.
    ///
    /// По этому и решается, звать ли мастер первоначальной настройки: файл есть —
    /// машину уже настраивали, чем бы ни закончилось. Проверять приходится до
    /// `load()`: тот на отсутствующем файле возвращает умолчания, и отличить
    /// «первый запуск» от «файл испорчен» после него уже нельзя.

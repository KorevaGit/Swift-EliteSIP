...
///
/// Тот же приём поедет в приложение: `NSMenu` там не прихоть, а единственный
/// способ показать список на Catalina, где `Menu` из SwiftUI ещё нет.
@MainActor
...
extension NSMenuItem {
    ...
    ///
    /// `nil` выключает пункт: у `NSMenuItem` без действия нет и активности, и
    /// это ровно то, что нужно «Отключён» в разговоре.
    var onSelect: (() -> Void)? {
    ...
    /// Держится в `representedObject`: у пункта меню цель слабая, и замыкание
    /// без такого хозяина освободилось бы до первого нажатия.

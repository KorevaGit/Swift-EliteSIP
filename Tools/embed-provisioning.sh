...
# Кода восстановления здесь больше нет: он лежал в бандле открытым текстом и
# открывал «Управление» всякому, кто вскрыл `.app`. Административный пароль
# остался — им машина живёт до первого захода на канал, после которого пароль
# приезжает из панели вместе с предустановкой.
if not str(config.get("adminPassword", "")).strip():
    sys.exit("пустой или отсутствующий ключ «adminPassword»")

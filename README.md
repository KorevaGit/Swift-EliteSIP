...
- [x] **M2** — исходящий звонок, RTP, аудиотракт, джиттер-буфер, раздельный выбор
...
- [x] **M2b** — SRTP с SDES для TLS: `RTP/SAVP`, `AES_CM_128_HMAC_SHA1_80`, запрет
      незаметного downgrade в обе стороны. Боевой Asterisk сейчас использует UDP,
      поэтому фактический защищённый профиль ещё не подтверждён выгрузкой его
      настроек — [docs/srtp.md](docs/srtp.md)
...
- [ ] **M7** — локальное администрирование и выпуск. Совместимость сама по себе
      ...
      Intel с Catalina, **M7g** отдельный аудио-DSP. Цепочка зависимостей одна:
      M7e → M7f → M7g

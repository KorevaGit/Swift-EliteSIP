...
| Окна | панель софтфона 332 pt · плавающее окно входящего · окно настроек |
...
- [ ] **M1.5** — FreePBX рядом с нашим Asterisk, чтобы видеть, что он
...
Поставить FreePBX «поверх» лабораторного Asterisk нельзя: FreePBX сам
генерирует `sip.conf`, `extensions.conf` и остальное из своей базы и затрёт наши
конфиги — а лаба нужна именно с предсказуемым руками написанным диалпланом.
Поэтому M1.5 — **второй, отдельный контейнер** с FreePBX и его собственным
Asterisk. Задача у него другая: не отладка клиента, а возможность посмотреть,
какой диалплан FreePBX создаёт под очередь раздачи и запись разговоров, и
сверить с тем, что видно в боевых CDR и CEL.

...
public enum SDPNegotiator {
    ...
    ///
    /// G.711 первым, PCMU перед PCMA — так настроен боевой пир, и решение M2c
    /// это закрепляет: боевой `allow` идёт в порядке
    /// `(ulaw|alaw|gsm|g726|g722)`, Asterisk как отвечающая сторона выбирает
    /// кодек по своей очереди, и разговор идёт на G.711 независимо от того, что
    /// стояло первым у нас. Широкую полосу на сервере решено не поднимать: в
    /// город всё равно уходит G.711 через транк, а лишний транскодинг платится
    /// процессорным временем сервера на каждом плече.
    ///
    ...
    /// широкополосным там, где стороны договариваются между собой, — и
    /// сохраняет уже сделанную и проверенную работу M2 годной к употреблению,
    /// если решение по серверу когда-нибудь пересмотрят.
    public static let defaultCodecs: [AudioCodec] = [.pcmu, .pcma, .g722]
    ...
    /// его уважает. Сам порядок задаёт `defaultCodecs`: G.711 первым как то, на
    /// чём разговор пойдёт на самом деле, широкая полоса следом как возможность.

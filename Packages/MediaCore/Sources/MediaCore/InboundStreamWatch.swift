...
public struct InboundStreamWatch: Sendable {
    ...
    public enum State: Sendable, Hashable {
        ...
        /// Состояние без подробностей — то, по чему считается «изменилось».
        ///
        /// Отдельный тип понадобился после живого прогона 18 августа 2026:
        /// в архиве для поддержки 62 строки из 266 оказались одним и тем же
        /// предупреждением, повторённым двадцать раз в секунду. Причина — само
        /// `State`: в нём лежит счётчик секунд, он растёт на каждом опросе, и
        /// сравнение «состояние то же?» отвечало «нет» всегда. Замысел был
        /// обратный — сказать один раз на переходе.
        ///
        /// Секунды при этом никуда не делись: они нужны в самом сообщении,
        /// просто не участвуют в решении, говорить ли.

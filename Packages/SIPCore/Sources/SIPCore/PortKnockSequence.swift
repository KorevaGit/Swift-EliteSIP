...
///
/// Признак удалённой работы — адрес сервера, а не адрес рабочего места.
/// В офисе в профиль вписывают внутренний `192.168.1.2`, снаружи — внешний
/// домен; проверять при этом собственный адрес машины было бы хуже, потому что
/// он у сотрудника тоже приватный (`192.168.1.154` в дампе боевого пира — это
/// его домашняя сеть, а не офисная), и различить по нему офис от дома нельзя
/// в принципе.
public enum PortKnockPolicy {
    ...
    public static func needsKnocking(serverHost: String) -> Bool {
        !isInternal(host: serverHost)
    }
}
